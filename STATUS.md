...
- **V2.6.x Hotfix (2026-05-08)** **Homepage merchant card productCount bug** (PR #57) — user-reported "件商品 數量對不起來". Two compounded bugs: (1) Drizzle subquery `${table.col}` interpolation outputs BARE column names → in correlated subqueries, `WHERE ${products.tenantId} = ${merchants.id}` resolved to `products.tenant_id = products.id` (inner-scope `id`) → always false → productCount + GMV subqueries silently returned 0 since V1; (2) productCount didn't filter `is_published = true` so even after fixing #1 it'd mismatch the storefront's published-only product list. Fix: rewrote subqueries with literal qualified names + added is_published filter + same pattern in `getPlatformStats` via inner-join (also filters out suspended/unapproved tenants from hero "件商品" stat). 3 regression tests in `tests/platform/featured-merchants.test.ts`. 304 → 307 tests (drift-checker static count); 324 → 327 tests at vitest runtime.
- **V2.6.x i18n (2026-05-08)** **Code + config comments unified to English** per user directive 「程式碼和設定檔中的註釋，統一用英文」. Two-PR sweep: PR #59 phase 1 (28 files: lib/ + db/ + root configs), phase 2 (137 files: app/ + components/ + tests/ + scripts/ + drizzle/ migrations + workflows + middleware + inngest workers + hooks). Total CJK reduction: 3,210 → 1,435 lines (-55%); files with any CJK: 177 → 141. **Zero comment-CJK remains** in the codebase. Preserved CJK: UI strings in JSX, brand voice prompts, product category enums (`服飾配件` etc.), test descriptions for readable failure output, fixture data (akami/afen brand voices), operator-facing console output, regex patterns matching Chinese, error messages, xlsx/CSV header arrays. 3 blog snippets in `compile-time-tenant-isolation.md` updated to match new English source comments (drift checker T4 caught all 3). Pure comment translation; pnpm typecheck + lint + lint:docs + vitest (327 tests) all green.
