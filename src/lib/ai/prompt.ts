 ...
 *
 * 來源：engineering-handoff-specs §2.1
 *
 * 注意：
 * - SYSTEM_PROMPT_TEMPLATE 內含 {{SANITIZED_BRAND_VOICE}} 占位符
 * - 一定要走 buildSystemPrompt() 而不是直接 string template，因為
 *   它會把 brand_voice 過 sanitizeBrandVoice() 再塞進 XML wrap
 * - 整個 prompt ~620 tokens，給 GPT-4o 還算便宜
 */
...
/**
 * 把 brand_voice 消毒過、替換進 template，產出最終 system prompt 字串。
 *
 * @param brandVoice 商家自填的風格指引（任意自由文字）
 * @returns 可直接丟進 generateObject({ system: ... }) 的字串
 */
export function buildSystemPrompt(brandVoice: string | null | undefined): string {
  const sanitized = sanitizeBrandVoice(brandVoice);
  // 空字串時給個合理 default，避免 prompt 出現空 XML
  const finalBrandVoice = sanitized || '（商家未提供，請使用中性、清楚、不浮誇的台灣電商常見語氣）';
  return SYSTEM_PROMPT_TEMPLATE.replace('{{SANITIZED_BRAND_VOICE}}', finalBrandVoice);
}
