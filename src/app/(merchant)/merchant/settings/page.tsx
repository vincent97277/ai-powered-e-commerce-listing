...
export default async function SettingsPage() {
  ...
  // 撈完整 merchant (含 themeVars). V2.6: dbUser 即可 — merchants 表對
  // web_anon 開放 SELECT (no RLS policy), 商家自己看自己這條 row 不需要
  // BYPASSRLS. 寫入路徑 (settings/actions.ts) 仍走 dbAdmin。
  const [m] = await dbUser
    .select()
    .from(merchants)
