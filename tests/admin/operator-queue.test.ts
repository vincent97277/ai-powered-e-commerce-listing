 ...
 *
 * 5 cases:
 *   1. Severity ordering: P1 paid_unshipped 排在 P3 no_photo 前
 *   2. 一個商家 3 種 signal → 出 3 個 QueueItem (不 collapse)
 *   3. 全部商家健康 → 回 [] (對應 EmptyState 顯示「全部商家狀態良好」)
 *   4. Suspended merchant 完全不出現在 queue
 *   5. Cross-tenant isolation: tenant A 的 signal 不會 bleed 到 tenant B 的 QueueItem
 *
 * UUID 命名 (避開 cost-cap cc/dd, rls.e2e 99/aa, demo 11/22, admin-search 88...aaa1/aaa2):
 *   TENANT_C = eeeeeeee-...
 *   TENANT_D = ffffffff-...
 *
 * 用 dbAdmin (admin observability 範圍, BYPASSRLS 合法).
 */
...
describe('V1.6 A8 — getOperatorQueue', () => {
  it('severity ordering: P1 paid_unshipped 排在 P3 no_photo 前', async () => {
    ...
    // stock=50 故意高於 low_stock threshold (5), 避免同時觸發 P3 low_stock
    ...
    // P1 必須出現在 P3 之前 (整個 queue 比較; cItems 也是 sorted subsequence)
  ...
  it('一個商家 3 種 signal → 出 3 個 QueueItem (不 collapse 成 1)', async () => {
    ...
    // tenant C: 1 product 同時有 zero_stock + zero_price + no_photo (3 issues 在同一件商品)
    // 預期 → 3 個 QueueItem (per signal-type, 不 collapse 成 「1 件商品 3 issue」)
    await dbAdmin.insert(products).values({
      tenantId: TENANT_C,
      title: '商品標題長度足夠避開 short_title',
      description: 'desc',
    ...
    // P2 (zero_stock / zero_price) 必須排在 P3 (no_photo) 之前
  ...
  it('全部商家健康 (TENANT_C/D 都沒 issue) → queue 不含這兩家', async () => {
    ...
    // tenant C 完全 healthy: 1 件正常商品, 沒任何 active order
    await dbAdmin.insert(products).values({
      tenantId: TENANT_C,
      title: '完全正常的商品標題長度',
      description: 'desc',
      ...
      priceCents: 10000,
      stockQuantity: 50, // 高於 low_stock threshold (5)
      aiMetadata: STUB_AI_META,
    });
    // tenant D: 完全沒商品沒訂單
  ...
  it('suspended merchant 完全排除 — 即使有 issue 也不出現在 queue', async () => {
    ...
    // suspended tenant 塞一堆 issue: paid order + zero_stock product
    await dbAdmin.insert(products).values({
  ...
  it('cross-tenant isolation: tenant C signals do NOT bleed into tenant D items', async () => {
    ...
    // tenant C: paid_unshipped × 2 + zero_stock × 3
    await dbAdmin.insert(products).values([
      {
        tenantId: TENANT_C,
        title: 'C 的零庫存商品 1',
    ...
    // tenant D: 只有 1 個 short_title issue (P4)
    await dbAdmin.insert(products).values({
      tenantId: TENANT_D,
    ...
    // tenant C: 2 signals (paid_unshipped × 1 entry, count=2; zero_stock × 1 entry, count=3)
    expect(cItems.length).toBe(2);
    ...
    // tenant D: 只有 1 個 short_title (P4)
